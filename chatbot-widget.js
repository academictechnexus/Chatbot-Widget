/* chatbot-widget.js - visual restored to provided image + full feature set
   Keep this in same folder as chatbot-widget.css
   Include as:
...
(function () {
    ...
    try {
      ...
      if (resp.ok && j && j.site) {
        ...
        if (activeSite.status === "demo") {
          ...
          // show remaining in header small badge (we use title area)
  ...
  // ---- feature UI per plan: we keep it minimal (you can extend) ----
  function renderFeatureArea() {
    // this widget's header area shows demo count; we keep feature area minimal to match image
    // if advanced/pro: show small hint as a last message
  ...
  function showUpgrade(url) {
    addBot(`Upgrade: ${url || (API_BASE + "/upgrade")}`);
  }
  ...
  // file input events & keyboard shortcut
  fileInput.addEventListener("change", (e) => { const f = e.target.files && e.target.files[0]; if (f) uploadFile(f); fileInput.value = ""; });
  wrapper.addEventListener("keydown", (e) => { if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "u") { e.preventDefault(); fileInput.click(); } });
